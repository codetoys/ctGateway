//ProtocolSvrTunnel.cpp 隧道协议

#include "ProtocolSvrTunnel.h" 
...
public:
	...
	public:
		...
		string toJson()
		{
			...
			cJSON_AddStringToObject(cjson, "sn", g_Global.pCGwConfig->GetBaseInfo().sn.c_str());
...
void CTunnelBase::recv_ms(string const& encodeType, int ms, bool once)
{
	char buf[1024];
	CMyTime t;
	t.SetCurrentTime();
	while (t.GetTimeSpanMS() < ms)
	{
		...
		{//指定设备型号的特殊处理
			...
			{
				g_Global.pIPlatform->post_Message("push", (str+"处理失败").c_str());
			}
			else
			{
				g_Global.pIPlatform->post_Message("push", frame.toJson().c_str());
			}
		}
		else
		{//通用处理
			...
			CMyTime ts;
			ts.SetCurrentTime();
			g_Global.pIPlatform->AddPushData((long)m_pInertfaceDevice, deviveCode.c_str(), 0, "", "", ts.GetTS_ms(),
				false, 0
				, str.c_str(), 0, "");
			g_Global.pIPlatform->post_DataMessage((long)m_pInertfaceDevice, deviveCode.c_str(), false, "", 0);
		}
...
void CTunnelBase::send_recv(string const& encodeType, string const& str)
{
	int buflen = str.size();
	...
	if (buf)
	{
		...
		{
			thelog << "发送出错 " << this->m_pInertfaceDevice->interfaceCode << " " << this->m_pInertfaceDevice->_ProtocolSvr->getIProtocol()->getIProtocolParam()->ToString() << ende;
		}
		else
		{
			thelog << "发送成功" << endi;
...
void CTunnelBase::process_send_command()
{
...
void CTunnelBase::protocol_process_job()
{
	bool bConnecting = false;//连接中，循环尝试连接只在第一次上报
	while (true)
	{
		if (!g_Global.bPlatformConnected)
		{
			thelog << "平台尚未连接" << ende;
			SleepSeconds(5);
			continue;
		...
		{
			thelog << "====================================================连接 " << this->m_pInertfaceDevice->interfaceCode << " " << this->m_pInertfaceDevice->_ProtocolSvr->getIProtocol()->getIProtocolParam()->ToString() << endi;
			if(!bConnecting)g_Global.pIPlatform->post_ReportWithFunc("connecting", "正在连接", m_pInertfaceDevice->interfaceCode.c_str());
			bConnecting = true;
			if (!pConnection->EnsureConnected(this->m_pInertfaceDevice))
			{
				thelog << "====================================================连接失败 " << this->m_pInertfaceDevice->interfaceCode << " " << this->m_pInertfaceDevice->_ProtocolSvr->getIProtocol()->getIProtocolParam()->ToString() << ende;
				...
				thelog << msg << ende;
				//g_Global.pIPlatform->post_ReportWithFunc("connecting_failed", msg.c_str(), m_pInertfaceDevice->interfaceCode.c_str());
				SleepSeconds(5);
				continue;
			}
			thelog << "====================================================连接成功 " << this->m_pInertfaceDevice->interfaceCode << " " << this->m_pInertfaceDevice->_ProtocolSvr->getIProtocol()->getIProtocolParam()->ToString() << endi;
			g_Global.pIPlatform->post_ReportWithFunc("connected", "连接成功", m_pInertfaceDevice->interfaceCode.c_str());
			bConnecting = false;
